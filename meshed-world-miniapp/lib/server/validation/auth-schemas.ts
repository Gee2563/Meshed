...
export const worldRegisterSchema = z.object({
  ...
  role: z.enum(["investor", "founder", "employee"]).default("founder"),
  verification: worldVerifySchema,
});

export const worldWalletAuthVerifySchema = z.object({
  address: z.string().regex(/^0x[a-fA-F0-9]{40}$/, "Expected an EVM wallet address."),
...
export const individualRegisterSchema = z.object({
  name: z.string().min(2),
  email: z.string().email(),
  role: z.enum(["investor", "founder", "employee"]),
  bio: z.string().min(10),
