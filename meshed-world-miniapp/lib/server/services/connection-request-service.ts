...
function seedRequestMessage(role: UserSummary["role"], currentUserName: string) {
  ...
  if (role === "founder") {
    return `I'm available to connect with ${currentUserName} on Meshed for the next founder support workflow.`;
  }
  return `Let's connect directly with ${currentUserName} on Meshed through the World-backed trust layer.`;
}
  ...
  return {
    ...
    async ensureDemoState(userId: string): Promise<ConnectionDashboardState> {
      ...
      const candidates = demoUsers
        ...
        .filter((user) => user.role === "investor" || user.role === "founder" || user.role === "employee")
