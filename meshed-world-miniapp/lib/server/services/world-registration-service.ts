...
import { roleForPersistence, roleLabel } from "@/lib/roles";
    ...
    async register(input: WorldRegisterInput): Promise<WorldRegisterResult> {
      ...
      if (existingUserId) {
        ...
        if (!sameNormalizedName(name, existingUser.name)) {
          throw new ApiError(
            409,
            `This World ID is already linked to ${existingUser.name}. Use that account or verify with a different World ID to create a new Meshed member.`,
          );
        }
        ...
        const onboardingProfile = await deps.onboardingRepository.upsertByUserId(existingUser.id, {
          ...
          mode: "INDIVIDUAL",
          title: roleLabel(existingUser.role),
